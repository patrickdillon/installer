...
func (i InfraProvider) Provision(dir string, parents asset.Parents) ([]*asset.File, error) {
	...
	if p, ok := i.capiProvider.(PreProvider); ok {
		...
		logrus.Debugf("No pre-provisioning requirements for the %s provider", i.capiProvider.Name())
	}
	...
	if p, ok := i.capiProvider.(InfraReadyProvider); ok {
		...
		logrus.Debugf("No infrastructure ready requirements for the %s provider", i.capiProvider.Name())
	}
	...
	if p, ok := i.capiProvider.(IgnitionProvider); ok {
		...
		logrus.Debugf("No Ignition requirements for the %s provider", i.capiProvider.Name())
	}
	...
	if p, ok := i.capiProvider.(PostProvider); ok {
...
// ignition secret.
func IgnitionSecret(ign []byte, infraID, role string) client.Object {
